@section('title')
    بوابة الحرية | قائمة صلة القرابة
@endsection
@section('style')
    ...
    <div class="page-meta">
        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">قائمة صلة القرابة</li>
            </ol>
        </nav>
    </div>
    <!-- /BREADCRUMB -->
            ...
            <div class="d-flex justify-content-between flex-wrap mt-2">
                ...
                <div>
                    <input wire:model.live="Search" type="search" id="Search"
                           placeholder="البحث في قائمة صلة القرابة...">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered text-center">
                    ...
                    @foreach($Relationships as $key => $row)
                        <tr>
                            <td>{{$Relationships->firstItem() + $key}}</td>
                            <td>{{$row->relationship_name ?? 'لا يوجد'}}</td>
                            <td>
            ...
            <div class="modal-content bg-white">
                <div class="modal-header {{$showEdit ? 'bg-warning': 'bg-primary'}}" style="margin: 5px;">
                    <h1 class="modal-title fs-5 text-white"
                        id="staticBackdropLabel">{{$showEdit ? 'تعديل صلة القرابة':'إضافة صلة قرابة'}}</h1>
                    <button type="button" class="btn-close bg-white" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form wire:submit.prevent="{{$showEdit ? 'updateRelationship':'createRelationship'}}">
                        <div class="row">
                            <div class="form-group col-md-12">
                                <label for="relationship_name">اسم صلة القرابة</label>
                                <input wire:model="state.relationship_name" type="text"
                                       class="form-control @error('relationship_name') is-invalid @enderror"
                                       id="relationship_name"
                                       placeholder="اسم صلة القرابة">
                ...
                <div class="modal-footer d-flex justify-content-start align-items-start">
                    <button type="submit" wire:click="{{$showEdit ? 'updateRelationship':'createRelationship'}}"
        ...
        <div class="modal-dialog">
            <div class="modal-content bg-white">
                <div class="modal-header bg-danger" style="margin: 5px;">
                    <h1 class="modal-title fs-5 text-white"
                        id="staticBackdropLabel">حذف صلة القرابة</h1>
                    <button type="button" class="btn-close bg-white" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <h5 class="text-danger m-3">هل أنت متأكد انك تريد حذف صلة القرابة؟</h5>
                    <span class="text-danger m-3">
                        * تنبية:
                        <span class="text-dark">
                            سيتم حذف {{$Relationships_->relationship_name ?? null}}
                        </span>
                    </span>
                </div>
                <div class="modal-footer d-flex justify-content-start align-items-start">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            document.body.addEventListener('hide_delete_modal', function () {
                Swal.fire(
                    {
                        title: 'نجاح',
                        text: 'تم حذف بيانات الصلة قرابة',
                        icon: 'success',
        ...
        document.addEventListener('DOMContentLoaded', function () {
            document.body.addEventListener('hideForm', function () {
                Swal.fire(
                    {
                        title: 'نجاح',
                        text: 'تم حفظ بيانات الصلة قرابة',
                        icon: 'success',
                        confirmButtonText: 'تم'
                    }
                );
