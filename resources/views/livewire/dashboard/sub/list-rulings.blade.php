    ...
    <div class="page-meta">
        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">قائمة الأحكام</li>
            </ol>
        </nav>
    </div>
    <!-- /BREADCRUMB -->
        ...
        <div class="col-md-12">
            <div class="d-flex justify-content-between flex-wrap mt-2">
                ...
                <div>
                    <input wire:model.live="Search" type="search" id="Search"
                           placeholder="البحث في قائمة الأحكام...">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered text-center">
                    ...
                    <tbody>
                    @foreach($Rulings as $key => $row)
                        <tr>
                            <td>{{$Rulings->firstItem() + $key}}</td>
                            <td>{{$row->ruling_name ?? 'لا يوجد'}}</td>
                            <td>
            ...
            <div class="modal-content bg-white">
                <div class="modal-header {{$showEdit ? 'bg-warning': 'bg-primary'}}" style="margin: 5px;">
                    <h1 class="modal-title fs-5 text-white"
                        id="staticBackdropLabel">{{$showEdit ? 'تعديل الحكم':'إضافة حكم'}}</h1>
                    <button type="button" class="btn-close bg-white" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form wire:submit.prevent="{{$showEdit ? 'updateRuling':'createRuling'}}">
                        <div class="row">
                            <div class="form-group col-md-12">
                                <label for="ruling_name">اسم الحكم</label>
                                <input wire:model="state.ruling_name" type="text"
                                       class="form-control @error('ruling_name') is-invalid @enderror"
                                       id="ruling_name"
                ...
                <div class="modal-footer d-flex justify-content-start align-items-start">
                    <button type="submit" wire:click="{{$showEdit ? 'updateRuling':'createRuling'}}"
        ...
        <div class="modal-dialog">
            <div class="modal-content bg-white">
                <div class="modal-header bg-danger" style="margin: 5px;">
                    <h1 class="modal-title fs-5 text-white"
                        id="staticBackdropLabel">حذف الحكم</h1>
                    <button type="button" class="btn-close bg-white" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <h5 class="text-danger m-3">هل أنت متأكد انك تريد حذف الحكم؟</h5>
                    <span class="text-danger m-3">
                        * تنبية:
                        <span class="text-dark">
                            سيتم حذف {{$Rulings_->ruling_name ?? null}}
                        </span>
                    </span>
                </div>
                <div class="modal-footer d-flex justify-content-start align-items-start">
